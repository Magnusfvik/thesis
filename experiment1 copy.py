...
def distance_from_expected_proper(track_styles, E_u_ids, tracks_df, sample_size=50):
    ...
    if len(E_u_list) > sample_size:
        # ADD SEED HERE FOR REPRODUCIBILITY
...
for idx, user_id in enumerate(sample_users):
    ...
    E_u_cat_set = user_E_u[user_id]['E_u_categories']
    user_test = test_sample[test_sample['user_id'] == user_id]
    
# In Step 6, replace track-to-track distance with category-level distance:
    ...
    for _, row in user_test.iterrows():
        ...
        # Get track categories (already parsed and split)
        track_categories = track_info.iloc[0]['category_styles']
        
        # Convert to set, splitting compounds
        track_cat_set = set()
        if isinstance(track_categories, list):
            for cat in track_categories:
                if '|' in cat:
                    track_cat_set.update([c.strip() for c in cat.split('|')])
                else:
                    track_cat_set.add(cat)
        
        # Get user's E_u categories (already built)
        E_u_cat_set = user_E_u[user_id]['E_u_categories']
        
        if not track_cat_set or not E_u_cat_set:
            continue
        
        # Simple category-level Jaccard distance
        distance = jaccard_distance(track_cat_set, E_u_cat_set)
...
# CRITICAL: Only use bins with ≥20 samples to avoid outliers
MIN_SAMPLES = 20
...
for i, bin_label in enumerate(bin_labels):
    if (bin_label in bin_stats.index and 
        bin_stats.loc[bin_label, 'sample_size'] >= MIN_SAMPLES):  # ← ADD THIS
        midpoint = 0.05 + i * 0.1
        avg_rating = bin_stats.loc[bin_label, 'avg_rating']
        bin_midpoints.append(midpoint)
        avg_ratings_list.append(avg_rating)

print(f"  ✓ Using {len(bin_midpoints)} bins with ≥{MIN_SAMPLES} samples")
